...
class WearMessagingClient(
    ...
    override fun connectToNode(nodeId: String): Flow<MessagingAction> {
        ...
        return callbackFlow {
            val listener: (MessageEvent) -> Unit = { event ->
                if (event.path.startsWith(BASE_PATHMESSAGING_ACTION)) {
    ...
    override suspend fun sendOrQueueAction(action: MessagingAction): EmptyResult<MessagingError> {
        return connectedNodeId?.let {
            try {
                ...
                client.sendMessage(it, BASE_PATHMESSAGING_ACTION, json.toByteArray())
                Result.Success(Unit)
    ...
    companion object {
        private const val BASE_PATHMESSAGING_ACTION = "runique/messaging_action"
    }
}